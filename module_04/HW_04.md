...
A_5 = np.array([[0, np.sin(a), 0, 1, 0, 0],
               ...
               [-1, 0, np.cos(a), 0, 0, 0],
               [0, 0, np.sin(a), 0, 0, 1]])
#Swap rows 1 and 4 to avoid pivot error

A_5 = np.array([[-1, 0, np.cos(a), 0, 0, 0],
               [1, np.cos(a), 0, 0, 1, 0],
               [0, np.cos(b/2), np.cos(b/2), 0, 0, 0],
               [0, -np.sin(b/2), np.sin(b/2), 0, 0, 0,],
               [0, np.sin(a), 0, 1, 0, 0],
...
b_arr_gen = [np.array([0,0,F*np.cos(theta),-F*np.sin(theta),0,0]) for F in F_arr]

#Solve using solveLU()
solved_gen = np.zeros((len(F_arr),6))
for i,b in enumerate(b_arr_gen):
    solved_gen[i]=solveLU(L_gen, U_gen, P_gen.T@b) 

#Plot All Reaction Forces
plt.figure(1,figsize=(8,5))
plt.plot(F_arr,solved_gen,label=['$P_1$','$P_2$','$P_3$','$R_1$','$R_2$','$R_3$'])
plt.title('Truss Reactions vs. Input Force')
plt.xlabel('Input Force (N)')
plt.ylabel('Truss Reaction (N)')
plt.legend();
...
#Set up Z matrix
yr = US["Year"]
Z_1 = np.block([[yr], [yr**0],[yr-1970], [yr-1970]])
...
plt.plot(t,Z_1@USsol, 'r--',label='US Fit')
...
plt.plot(t,Z_1@EUsol, 'r--',label='US Fit')
plt.xlabel("Year")
plt.ylabel("Energy Consumption (TW-Hrs)")
