...
#Plotting regular
plt.figure(1)
plt.plot(beers_cleaned["ibu"],beers_cleaned["abv"], "o",label="Cleaned Data")
plt.legend()
plt.figure(2)
plt.plot(beers_filled["ibu"],beers_filled["abv"], 'go',label='Filled NaNs with 0')
plt.legend()
```

## End of Part b: By comparing the graphs above it can be noticed that cleaning the data removes a large category of beer with no IBUs. 
