...
for i,step in enumerate(steps):
    ...
    plt.plot(x[:-1], newT, label='dx = {}'.format(step))
    plt.loglog(x[:-2], -np.diff(newT)*k_fin, label='heat flux')
    plt.xlabel("Position $x$")
    plt.ylabel("Temperature $T^{\prime}(x)$")
...
```{code-cell} ipython3
d4ohm = (ohm[4:]-4*ohm[3:-1]+6*ohm[2:-2]-4*ohm[1:-3]+ohm[:-4])/step**4
```
