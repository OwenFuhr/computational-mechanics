...
```{code-cell} ipython3
def PercentNaN(set1, set2):
    return (len(set1.values)-len(set1.dropna().values))/len(set1.values)*100, (len(set2.values)-len(set2.dropna().values))/len(set2.values)*100

abv = beers['abv']
ibu = beers['ibu']
abvNAN, ibuNAN = PercentNaN(abv, ibu)

print("The perentage of NaNs in these data sets is {:.2f} % for ABV and {:.2f} % for IBU".format(abvNAN, ibuNAN))
```
